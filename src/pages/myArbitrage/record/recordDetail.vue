  ...
  <div class="achievement">
        ...
        <div class="wrap">
          <div class="flex-box">
            ...
            <div class="right-content">
              <div class="bread">
                <router-link to="/account/billingRecord" tag="span" style="cursor: pointer">{{$t('billingRecord.text1')}}</router-link>
                <i> > </i>
                <span>{{$t('billingRecord.text16')}}</span>
              </div>
              <ul class="pcDetail">
                <li><span>{{$t('billingRecord.text3')}}</span><i>{{getAbs(recordDetail.amount)}} {{recordDetail.currency}}</i></li>
                <li><span>{{$t('billingRecord.text17')}}</span><i>{{arname}}</i></li>
                <li><span>{{$t('billingRecord.text18')}}</span><i>{{arid}}</i></li>
                <li><span>{{$t('billingRecord.text19')}}</span><i>{{parsePaymentType(recordDetail.paymentType)}}</i></li>
                <li><span>{{$t('billingRecord.text20')}}</span><i>{{recordDetail.dingdanid}}</i></li>
                <li><span>{{$t('billingRecord.text21')}}</span><i v-if="recordDetail.createAt">{{$changeDate(recordDetail.createAt, '/')}}</i></li>
              </ul>
            </div>
          </div>
        </div>
    ...
    <div class="billing-app">
      <div class="billing-head">
        <router-link tag="div" to="/account/billingRecord" class="back"></router-link>
        <div class="title">{{$t('billingRecord.text16')}}</div>
      </div>
      <div class="recordDetail">
        <div class="account">
          <p class="num">{{getAbs(recordDetail.amount )}} {{recordDetail.currency}}</p>
          <p>{{$t('billingRecord.text3')}}</p>
        </div>
        <div class="cont">
          <p><span>{{$t('billingRecord.text17')}}</span><span>{{arname}}</span></p>
          <p><span>{{$t('billingRecord.text18')}}</span><span>{{arid}}</span></p>
        </div>
        <div class="cont">
          <p><span>{{$t('billingRecord.text19')}}</span><span>{{parsePaymentType(recordDetail.paymentType)}}</span></p>
          <p><span>{{$t('billingRecord.text20')}}</span><span>{{recordDetail.dingdanid}}</span></p>
          <p><span>{{$t('billingRecord.text21')}}</span><span>{{$changeDate(recordDetail.createAt, '/')}}</span></p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  mounted () {
    // console.log('哈哈哈哈哈哈哈哈', this.$route.query.id)
    var langs = window.localStorage.getItem('Qlang')
    if (langs) {
      this.lang = langs
    } else {
      this.lang = 'CN'
  ...
  methods: {
    ...
    getTransactionDetail () {
      let url = `${this.$api}/trade/transactionDetail?id=${this.id}`
      this.$http.post(url).then(res => {
        if (res.data.success) {
          this.recordDetail = res.data.data
          this.arid = res.data.mapData.arid
          if (this.lang === 'CN') {
            this.arname = res.data.mapData.arname
          } else {
            this.arname = res.data.mapData.remark
          }
        }
      }).catch(err => {
        console.log(err)
