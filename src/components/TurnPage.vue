<template>
  <div class="turnPage" v-if="pages !== 0">
    <div class="pagelist">
      <span class="phTurnPage" :class="{disabled: pstart}" @click="jumpPage(--current_page)">&lt; {{$t('pageTurn.text1')}}</span>
      ...
      <span class="phTurnPage" :class="{disabled: pend}" @click="jumpPage(++current_page)">{{$t('pageTurn.text2')}} &gt;</span>
    </div>
  </div>
</template>
...
export default{
  ...
  methods: {
    jumpPage (page) {
      ...
      this.$emit('arbitrageTrunPage', page) // 套利订单进行中、已完成
      this.$emit('profitTrunPage', page) // 收益记录
      this.$emit('accumulativeTrunPage', page) // 业绩累计
      this.$emit('orderLogTrunPage', page) // 已完成详情订单
      this.$emit('isOverTrunPage', page) // 已完成详情订单
      this.$emit('parentTurnPage', page)
    },
    resetPage (val) {
